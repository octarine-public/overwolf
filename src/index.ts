...
EventsSDK.on("SharedObjectChanged", (id, reason, obj) => {
	if (id === SOType.GameAccountClient) {
		selfAccountID = 76561197960265728n + BigInt(obj.get("account_id") as number)
	}
	// if (id === 2003) {
...
function RealodGUIData() {
	if (!state.value || selfAccountID === undefined || selfAccountID === undefined) {
		return
	}
	if (!dataSleeper.Sleeping("ReloadData")) {
		currentPlayersCache.clear()
...
function GetGUIAcceptButton(baseData: GUIBaseData): Rectangle {
	const acceptTextSize = Vector2.FromVector3(RendererSDK.GetTextSize("ACCEPT"))
...
function RenderHeroStats(rect: Rectangle, heroData: HeroData, fontSize = 16): void {
...
function ShowTooltip(rect: Rectangle, cursor: Vector2, tooltip: string): void {
	...
	RendererSDK.Text(
		tooltip,
		Position.AddForThis(tooltipBorderSize).AddScalarX(SizeImage.x + Addscalar),
		Color.White,
		tooltipFont,
		tooltipFontSize
	)
...
EventsSDK.on("Draw", () => {
	...
	if (!state.value || currentLobbyMembers === undefined) {
		return
	}
	for (const member of currentLobbyMembers) {
		requestPlayerDataIfEnabled(member.get("id") as bigint)
	}
	if (!panelShown || currentPlayersCache.size === 0) {
		return
	}

	const cursor = Input.CursorOnScreen
	const guiBaseData = GetGUIBaseData()
	RendererSDK.FilledRect(guiBaseData.rect.pos1, guiBaseData.rect.Size, backgroundColor)
...

	{
		...
		RendererSDK.FilledRect(guiCloseButton.pos1, guiCloseButton.Size, closeButtonColor)
	...
	if (sendPing) {
		...
		RendererSDK.Text(
			"ACCEPT",
			new Vector2(
				guiAcceptButton.pos1.x + guiAcceptButton.Width / 2 - acceptTextSize.x / 2,
	...
	for (let i = 0, end = currentLobbyMembers.length; i < end; i++) {
		...
		if (i === 5) {
			...
			RendererSDK.FilledRect(
				new Vector2(playerRect.pos1.x, playerRect.pos2.y + playerSeparatorOffset),
		...
		if (latestPartyID === member.get("party_id")) {
			const prevI = i - 1
			const prevRect = GetPlayerRect(guiBaseData, prevI >= 5 ? prevI + 1 : prevI)
...

		{
			const rectTotalMatches = new Rectangle(
				currentPos,
				new Vector2(rect.pos1.x + separatorTotalMatchesOffset.x, rect.pos2.y)
			)
		...
		const userRecentOutcomes = ExtractOutcomesInfo(data[0].recent_outcomes)
		{
			const rectLastInfo = new Rectangle(
				currentPos,
				new Vector2(rect.pos1.x + separatorLastInfoOffset.x, rect.pos2.y)
			)
...

		{
			const rectLastCommends = new Rectangle(
				currentPos,
				new Vector2(rect.pos1.x + separatorLastCommendsOffset.x, rect.pos2.y)
			)
...

		{
			const rectLastPickedHeroes = new Rectangle(
				currentPos.Clone(),
				new Vector2(rect.pos1.x + separatorLastPickedHeroesOffset.x, rect.pos2.y)
			)
...
InputEventSDK.on("MouseKeyDown", mask => {
	...
	const cursor = Input.CursorOnScreen
	const guiBaseData = GetGUIBaseData()
	if (!guiBaseData.rect.Contains(cursor)) {
		return true
	}
